...
// import java.lang.*; 보이지 않지만 기술돼 있음
...
public class ArrayCopy {
	...
	public void method2() {
		...
		// 깊은복사 방법2. System클래스에서의 arraycpoy() 메소드를 이용
		public void method3() {
			int[] origin = {1, 2, 3, 4, 5,};
			...
			// 2. arraycopy 메소드 실행
			//		System.arraycopy(원본배열, 복사시작할인덱스, 복사본배열, 복사본배열의복사될시작인덱스, 복사할갯수);
			...
			System.out.println("\norigin의 주소값 : " + origin.hashCode());
			System.out.println("cpoy의 주소값 : " + copy.hashCode());
			//	주소값이 다름
			//	== 깊은복사
			//  == 다른 배열을 참조
		...
		public void method4() {
			...
			/*
			 * 내가 전달할 복사할 길이만큼의 크기로 배열 생성
			 * 		이때 해당 복사할 길이가 원본배열의 크기보다 클 경우		=> 원본데이터 전체 복사
			 * 		이때 해당 복사할 길이가 원본배열의 크기보다 작을 경우	=> 원본데이터 중에 해당 갯수만큼 복사
			 * 		
			 */
			for(int i=0; i<copy.length; i++) {
				System.out.print(copy[i] + " ");
		...
		public void method5() {
			...
			*/
			// * Arrays.toString(배열) : 배열의 각 인덱스의에 담긴값을 한 문자열로 합쳐서 반환 (앞과뒤에 [] 감싸줌, ,구분자로 연이어줌)
		 ...
		 * java.util.Arrays
		 * ...
		 * => java.lang 이외의 패키지에 있는 클래스들을 사용할 때는 import를 사용 해야됨.
		 */
}
